...
typedef enum CatalogTable {
	HYPERTABLE = 0, // 专门用来记录 转换成hyperTable表的表
...
typedef struct CatalogTableInfo {
	...
	const char *name;
	Oid id;							   // 表的oid
	Oid serial_relid;				   // 对应sequence的oid
	Oid index_ids[_MAX_TABLE_INDEXES]; // 使用的多个index的oid
} CatalogTableInfo;
...
extern void ts_catalog_table_info_init(CatalogTableInfo *catalogTableInfoArr,
									   int maxTableCount,
